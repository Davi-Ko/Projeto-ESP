...
void setupWebServer() {
    ...
    server.on("/api/device/setSyncMode", HTTP_POST, handleSetSyncMode);
    server.on("/api/device/triggerTwinSync", HTTP_POST, handleTriggerTwinSync);
...
void handleSetSyncMode() {
    ...
    message.command = 15;
    message.syncMode = syncMode;
    message.isAutoSync = false;
    strcpy(message.message, "SET_SYNC_MODE");

    esp_now_send(devices[deviceIndex].macAddress, (uint8_t *)&message, sizeof(message));

    server.sendHeader("Access-Control-Allow-Origin", "*");
    server.send(200, "text/plain", "Sync mode updated");
}

void handleTriggerTwinSync() {
    if (!server.hasArg("plain")) {
        server.send(400, "text/plain", "Body not found");
        return;
    }
    ...
    message.command = 16; 
    message.isAutoSync = false;
    strcpy(message.message, "FORCE_TWIN_SYNC");

    esp_now_send(devices[deviceIndex].macAddress, (uint8_t *)&message, sizeof(message));

    server.sendHeader("Access-Control-Allow-Origin", "*");
    server.send(200, "text/plain", "Twin sync triggered");
}
...
void handleGetDevices() {
    ...
    for (int i = 0; i < deviceCount; i++) {
        ...
        device["syncMode"] = devices[i].syncMode;
        device["isTwinDevice"] = devices[i].isTwinDevice;
...
void OnDataRecv(uint8_t *mac, uint8_t *incomingData, uint8_t len) {
    ...
    Serial.print(message.syncMode);
    Serial.print(", IsAutoSync: ");
    Serial.println(message.isAutoSync ? "YES" : "NO");

    if (message.command == 10 || message.command == 11 || message.command == 12) {
        updateDeviceInfo(message.deviceId, mac, message.relayState, message.message, message.syncMode);
    }
}
...
void updateDeviceInfo(uint8_t deviceId, uint8_t *mac, bool relayState, const char *deviceType, uint8_t syncMode) {
    ...
    if (index != -1) {
        ...
        devices[index].syncMode = syncMode;
        // Detectar se é twin relay baseado no deviceType
        devices[index].isTwinDevice = (strstr(deviceType, "TWIN") != NULL);
        ...
        Serial.print(", SyncMode=");
        Serial.println(syncMode == 0 ? "TOGGLE" : "MIRROR");
    }
}
